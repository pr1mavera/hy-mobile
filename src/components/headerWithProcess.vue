<template>
  <div class="headerWithProcess border-1px-b">
    ...
    <div class="process">
      ...
      <span tag="span" :class="['processItem', $route.name === 'selectTicket'? 'title-black' : '']">选择门票</span>
      ...
      <span tag="span" :class="['processItem', $route.name === 'fillInTicketMsg'? 'title-black' : '']">填写信息</span>
      <svg class="icon" aria-hidden="true">
          <use xlink:href="#icon-you2"></use>
      </svg>
      <!-- <router-link tag="span" class="processItem" :to="{ path: 'success', name: 'success'}">购买成功</router-link> -->
      <span tag="span" :class="['processItem', $route.name === 'success'? 'title-black' : '']">购买成功</span>
    </div>
  </div>
</template>
